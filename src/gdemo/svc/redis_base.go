...
func (r *RedisBaseSvc) SaveJsonDataToRedis(key string, v interface{}, expireSeconds int64) error {
	...
	if err != nil {
		r.WarningLog(LogKindRedisBaseSvc, []byte("json_encode "+key+" error: "+err.Error()))
		return err
	}
	...
	if err != nil {
		r.Rclient.Free()
		r.WarningLog(LogKindRedisBaseSvc, []byte("set "+key+" to redis error: "+err.Error()))
		return err
	}
...
func (r *RedisBaseSvc) GetJsonDataFromRedis(key string, v interface{}) (bool, error) {
	...
	if reply.Err != nil {
		r.Rclient.Free()
		r.WarningLog(LogKindRedisBaseSvc, []byte("get "+key+" from redis error: "+reply.Err.Error()))
	...
	if err != nil {
		r.WarningLog(LogKindRedisBaseSvc, []byte("reply "+key+" from redis error: "+err.Error()))
		return false, err
	}

	err = json.Unmarshal(jsonBytes, v)
	if err != nil {
		r.WarningLog(LogKindRedisBaseSvc, []byte("json_decode "+key+" from redis error: "+err.Error()))
		return false, err
	}
...
func (r *RedisBaseSvc) SaveHashEntityToRedis(key string, entityPtr interface{}, expireSeconds int64) error {
	...
	if len(errIndexes) != 0 {
		...
		r.WarningLog(LogKindRedisBaseSvc, []byte(msg))
...
func (r *RedisBaseSvc) GetHashEntityFromRedis(key string, entityPtr interface{}) (bool, error) {
	...
	if reply.Err != nil {
		r.Rclient.Free()
		r.WarningLog(LogKindRedisBaseSvc, []byte("hgetall "+key+" from redis error: "+reply.Err.Error()))
		return false, reply.Err
	}
	...
	if err != nil {
		r.WarningLog(LogKindRedisBaseSvc, []byte("reply to struct "+key+" from redis error: "+err.Error()))
		return false, err
	}
