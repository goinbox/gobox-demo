...
func NewMongoDao(client *Client) *MongoDao {
	return &MongoDao{
		client,
	}
}
...
func (m *MongoDao) SelectAll(tableName string, query *Query) ([]bson.M, error) {
	result := []bson.M{}
...
func (m *MongoDao) SelectOne(tableName string, query *Query) (bson.M, error) {
	result := bson.M{}
